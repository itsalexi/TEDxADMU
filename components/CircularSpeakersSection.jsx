  ...
  const speakers = [
    {
      ...
      position: 'north',
      details: 'Dr. Santos has conducted groundbreaking research on neural plasticity and consciousness, with over 50 publications in leading scientific journals. Her TED talk will explore the frontiers of human perception and how we navigate the labyrinths of our own minds.'
    },
    {
      ...
      image: '/api/placeholder/300/300',
      position: 'east',
      details: 'Juan Reyes has transformed urban spaces across South America, implementing biophilic design principles that bring nature into cities. His award-winning structures serve as both functional spaces and psychological sanctuaries, guiding people through mindful journeys in the concrete jungle.'
    },
    {
      ...
      image: '/api/placeholder/300/300',
      position: 'south',
      details: 'Ana Cruz founded ConnectAll, a social enterprise that has brought internet access to over 200 remote communities. Her innovative mesh network technology runs on renewable energy and has become a model for sustainable connectivity solutions worldwide.'
    },
    {
      name: 'Prof. David Lee',
      ...
      image: '/api/placeholder/300/300',
      position: 'west',
      details: 'Professor Lee combines Eastern and Western philosophical traditions to create new ethical frameworks for AI development and implementation. His work with major tech companies has influenced responsible design practices and policy recommendations for the ethical deployment of autonomous systems.'
    },
  ];
          ...
          <div className="relative w-full h-[600px] mb-16 flex items-center justify-center">
            ...
            >
              {speakers.map((speaker, index) => (
                <div
                  key={index}
                  className={`absolute bg-white rounded-md bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-10 border border-white-300 transition-all duration-500 shadow-lg cursor-pointer ${expandedSpeakerIndex === index ? 'overflow-y-auto' : 'h-64 w-56 overflow-hidden'}`}
                  style={getPositionStyles(index, speakers.length, false)}
                  onClick={() => handleSpeakerClick(index)}
                >
                  <div className={`w-full flex flex-col rounded-md ${expandedSpeakerIndex === index ? '' : 'h-full'}`}>
